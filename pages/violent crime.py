...
def load_violent_crime_dataset():
    df = pd.read_csv(r'datasets\violent_crimes\violent_crimes_(State-UT-wise)_2021.csv', index_col='Name')
...
fig4 = px.area(df, df.index, 'RateofViolentCrimes', title='Rate of Violent Crimes Data')
...
state_year_group = df.groupby('State/UT')[['midPopulation','RateofViolentCrimes','ChargesSheetingRate']].sum().reset_index()
fig6a = px.pie(state_year_group, 'State/UT', 'midPopulation' )
fig6b = px.pie(state_year_group, 'State/UT', 'RateofViolentCrimes' )
