...
def load_violent_crime_dataset():
    df = pd.read_csv(r'datasets\crime_women_children\crime_against_women_(State-UT-wise)_2019-2021.csv', index_col='Name')
...
fig4 = px.area(df, df.index, 'RateofCrimeAgainstWomen', title='Rate of crime against womens Data')
...
state_year_group = df.groupby('State/UT')[['midPopulation','RateofCrimeAgainstWomen','ChargesSheetingRate']].sum().reset_index()
fig6a = px.pie(state_year_group, 'State/UT', 'midPopulation' )
fig6b = px.pie(state_year_group, 'State/UT', 'RateofCrimeAgainstWomen' )
